...
def unique_values(wbid_shp):
    with arcpy.da.SearchCursor(wbid_shp, ['WBID']) as cursor:
        return sorted({row for row in cursor})

wbid_input = unique_values(waterbody_input)
wbid = [x.strip() for x in wbid_input[0]]
print(wbid)
...
for w in wbid:
    ...
    for p in param_split:
        ...
        LR_raw = LA.Analysis(df_raw)
        R_squared, P_value, y_predicted, x, slope, intercept = LR_raw.regression(p, y_axis_select)
        XY_raw = plt.XYPlot(w, df_raw, [p], y_axis_select, R_squared, y_predicted, x, slope, intercept, 'Raw Data')
        TS_raw = plt.TimeSeriesPlot(w, df_raw, 'Date', [p], 'Raw Data')
        
        ### AGMs ###
        df_AGM = water.qaFiltered(lakeWatch = 'No')
        
        LR_AGM = LA.Analysis(df_AGM)
        R_squared, P_value, y_predicted, x, slope, intercept = LR_AGM.regression(p, y_axis_select)
        XY_AGM = plt.XYPlot(w, df_AGM, [p], y_axis_select, R_squared, y_predicted, x, slope, intercept,'AGM')
        ...
        landuseMap = plt.mapFigure(r'C:\development_2\toolRuns\test_eric\multiple_tables\dashboard\PLSM_shapefiles\landuse_dissolveLTA_Loading.shp',
                                     r'C:\development_2\toolRuns\test_eric\multiple_tables\dashboard\Lake Lotta Wbid.shp',
                                     [p])
        
        db_raw = layout([[TS_raw], [XY_raw, pc], [landuseMap]], sizing_mode = 'scale_width')
        db_AGM = layout([[TS_AGM], [XY_AGM, pc], [landuseMap]], sizing_mode = 'scale_width')
